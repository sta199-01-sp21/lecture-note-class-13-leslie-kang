...
The downside is that you are less likely to be accurate as opposed to an
interval.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-leslie-kang/manhattan.csv")
```
...
We are 95% confident that that the true proportion of mice that died while not
on the treatment is between 0.690 and 0.797.
...
treatment is between 0.552 and 0.651.
